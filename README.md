...
1. [Bundle Annotations](#bundle-annotations)
    * [@Paginator](#paginator)
        ...
        * [Paginator Nulls](#paginator-nulls)
...
You can also define some fields to null. Is same as `wheres` section, but
specific for NULL assignments. The `nulls` section must be defined as an array
of arrays, and each array should contain these positions:
...
You can add route informations at form builder, using `routeName` and `routeParameters`.
The `method` can also be specified.
The setting `routeParameters` is an array with key as route arguments names and value can be single value or an array with `entity` and `method` settings
 ...
 *      name            = "userForm",
 *      validate        = "isValid",
 *      routeName       = 'valid_route_name',
 *      routeParameters = {
 *          "id": {"entity": "user", "method": "getId"},
 *          "page": 2,
 *      },
 *      method = "PUT"
 * )
